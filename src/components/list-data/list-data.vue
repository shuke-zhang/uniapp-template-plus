...
import type { ListActions, ListDataState } from '@/hooks/useListData'
...
<template>
  ...
  >
    ...
    <template v-else>
      ...
      <view v-if="state?.status === 'noMore'" class="load-more flex-center text-black-3">
        {{ state?.error ? `加载失败 点击重试` : '没有更多数据了' }}
      </view>
      <view v-if="state?.status === 'more'" class="load-more flex-center text-black-3">
        上拉显示更多
      </view>
      <view v-if="state?.status === 'loading'" class="load-more flex-center text-black-3">
        正在加载...
      </view>
      <view
        v-if="safeArea"
